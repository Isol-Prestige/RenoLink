...
const pageTitles = {
  ...
  factures:    { title: 'Factures',          sub: 'Historique des factures de commission' },
...
function openProject(id) {
  ...
  // Statut hidden + picker
  const hiddenSel = document.getElementById('m-statut-select');
  if (hiddenSel) hiddenSel.value = p.statut;
  refreshStatutPicker(p.statut);

  // Grand badge statut dans section suivi
  const bigBadge = document.getElementById('m-statut-badge-big');
  if (bigBadge) {
    const st0 = statutCfg[p.statut] || { text: p.statut, cls: '' };
    bigBadge.innerHTML = p.statut
      ? `<span class="statut ${st0.cls}" style="font-size:.8rem;padding:5px 14px;">${st0.text}</span>`
      : `<span style="font-size:.78rem;color:var(--muted);font-style:italic;">Aucun statut</span>`;
  }
...
// ══════════════════════════════════════════
// STATUT — CLIC TOGGLE (CŒUR DU FIX)
// ══════════════════════════════════════════

/**
 * Clic sur un badge statut dans le picker :
 * - Si le badge est déjà actif → on supprime le statut (remet à 'nouveau')
...
function clickStatut(val) {
  ...
  if (proj.statut === val) {
    // Re-clic sur statut actif = désélection → retour à nouveau
    appliquerStatut(proj, 'nouveau');
    toast('Statut réinitialisé → Nouveau', 'info');
  } else {
...
/**
 * Applique un statut et met à jour TOUTE l'interface
 */
function appliquerStatut(proj, newStatut) {
  ...
  // Grand badge dans la section suivi
  const bigBadge = document.getElementById('m-statut-badge-big');
  if (bigBadge) {
    bigBadge.innerHTML = newStatut
      ? `<span class="statut ${st.cls}" style="font-size:.8rem;padding:5px 14px;">${st.text}</span>`
      : `<span style="font-size:.78rem;color:var(--muted);font-style:italic;">Aucun statut</span>`;
  }
  ...
  // Picker — mettre en surbrillance le bon bouton
  ...
  // Historique
  proj.historique = proj.historique || [];
  proj.historique.push({
    a: 'Statut → ' + (st.text || newStatut),
    d: nowStr(),
    user: 'Admin'
  });
  renderTimeline(proj);
...
/**
 * Met en évidence visuellement le badge actif dans le picker
 */
...
/**
 * Bouton "✕ Supprimer statut" — remet à nouveau sans confirmation
 */
...
// Mise à jour montant signé
function updateMontantSigne() {
  const p = AppState.activeProject;
  if (!p) return;
  const proj = projects.find(x => x.id === p.id);
  if (!proj) return;
  const val = parseFloat(document.getElementById('m-montant-signe').value) || 0;
  proj.montantSigne = val;
  AppState.activeProject = proj;
  setEl('m-comm', Math.round(val * 0.08).toLocaleString('fr-FR') + ' €');
  renderKPIs();
}
...
function setStatutRapide(val) { clickStatut(val); }
...
function renderAssignedArtisan(p) {
  ...
  if (p.artisanAssigne) {
    ...
    el.innerHTML = `<p style="font-size:.78rem;color:var(--muted);padding:6px 0 10px;">Aucun artisan assigné.</p>`;
  }
}
...
function renderArtisanButtons(p) {
  ...
  el.innerHTML = actifs.map(a =>
    `<button class="mfbtn ${p.artisanAssigne === a.rs ? 'gold' : ''}"
      ...
      🔨 ${escHtml(a.rs)}${p.artisanAssigne === a.rs ? ' ✓' : ''}
    </button>`
  ).join('');
}
...
function handlePhotoUpload(input) {
  ...
  if (proj.photos.length + files.length > 10) {
    toast('⚠️ Maximum 10 photos par projet', 'error'); return;
  }
  let loaded = 0;
  files.forEach(file => {
    if (!['image/jpeg','image/png','image/webp','image/gif','image/heic'].includes(file.type.toLowerCase())) {
      toast('⚠️ Format non supporté : ' + file.name, 'error'); return;
    }
    if (file.size > 10 * 1024 * 1024) {
      toast('⚠️ Fichier trop lourd (max 10 Mo) : ' + file.name, 'error'); return;
    }
    const reader = new FileReader();
    reader.onload = (e) => {
      proj.photos.push({
        name: sanitize(file.name), dataUrl: e.target.result,
        size: file.size, type: file.type, addedAt: nowStr()
      });
      loaded++;
...
function openLightbox(index) {
  ...
  lb.innerHTML = `
    <div style="position:relative;max-width:90vw;max-height:88vh;display:flex;flex-direction:column;align-items:center;gap:12px;">
      ...
      <div style="color:rgba(255,255,255,.6);font-size:.78rem;">${escHtml(p.photos[index].name)} — ${(p.photos[index].size/1024).toFixed(0)} Ko</div>
      ${p.photos.length > 1 ? `<div style="display:flex;gap:12px;">
        <button onclick="event.stopPropagation();openLightbox(${index>0?index-1:p.photos.length-1})" style="background:rgba(255,255,255,.15);border:none;color:#fff;padding:8px 18px;border-radius:8px;cursor:pointer;">← Précédente</button>
        <button onclick="event.stopPropagation();openLightbox(${index<p.photos.length-1?index+1:0})" style="background:rgba(255,255,255,.15);border:none;color:#fff;padding:8px 18px;border-radius:8px;cursor:pointer;">Suivante →</button>
      </div>` : ''}
      <button onclick="document.getElementById('lightbox').remove()" style="position:absolute;top:-16px;right:-16px;width:32px;height:32px;border-radius:50%;background:#fff;border:none;cursor:pointer;font-size:1rem;display:flex;align-items:center;justify-content:center;">✕</button>
    </div>`;
...
function renderTimeline(p) {
  ...
  el.innerHTML = hist.map((h, i) => `
    <div class="tl-item">
      <div class="tl-dot ${i === hist.length - 1 ? 'active' : 'done'}">${i === hist.length - 1 ? '→' : '✓'}</div>
...
function saveNotes() {
  ...
  if (val.length > 2000) { toast('⚠️ Notes trop longues (max 2000 caractères)', 'error'); return; }
...
function rejectFromModal() {
  ...
  if (typeof EmailTemplates !== 'undefined' && EmailTemplates.rejete) {
...
function submitAddProject() {
  ...
  if (!type || !nom || !tel || !ville) { toast('⚠️ Remplissez les champs obligatoires', 'error'); return; }
  if (budget < 1500) { toast('⚠️ Budget minimum requis : 1 500 €', 'error'); return; }
  if (tel.length < 10 || !/^[0-9+\s]+$/.test(tel)) { toast('⚠️ Numéro de téléphone invalide', 'error'); return; }
  if (email && !isValidEmail(email)) { toast('⚠️ Adresse email invalide', 'error'); return; }
  if (cp && (cp.length !== 5 || isNaN(cp))) { toast('⚠️ Code postal invalide', 'error'); return; }
...
function submitAddArtisan() {
  ...
  if (!rs || !tel || !metier || !ville) { toast('⚠️ Champs obligatoires manquants', 'error'); return; }
  if (tel.length < 10) { toast('⚠️ Téléphone invalide', 'error'); return; }
  if (email && !isValidEmail(email)) { toast('⚠️ Email invalide', 'error'); return; }
  if (siret && (siret.length !== 14 || isNaN(siret))) { toast('⚠️ SIRET invalide (14 chiffres)', 'error'); return; }
...
function renderArtisans() {
  ...
  body.innerHTML = artisans.map(a => {
    ...
    <div class="artisan-row" onclick="openArtisan('${escAttr(a.id)}')" style="cursor:pointer;">
      ...
      <div class="art-tags">${tags||'<span style="font-size:.7rem;color:var(--muted);">Aucune certification</span>'}</div>
      ${badge}
      <div class="actions" style="margin-left:12px;" onclick="event.stopPropagation()">
        <button class="abtn" onclick="openArtisan('${escAttr(a.id)}')">👁️</button>
        <button class="abtn" onclick="window.location.href='tel:${escAttr(a.tel)}'">📞</button>
        <button class="abtn" onclick="waArtisan('${escAttr(a.id)}')">💬</button>
        ${a.statut === 'attente'
          ? `<button class="abtn" onclick="validateArtisan('${escAttr(a.id)}')">✅</button>`
          : `<button class="abtn danger" onclick="suspendArtisan('${escAttr(a.id)}')">⏸️</button>`}
      </div>
    </div>`;
  }).join('');
}
...
function waArtisan(id) {
  ...
  if (p && typeof EmailTemplates !== 'undefined') {
...
function renderCommissions() {
  ...
  wrap.innerHTML = `
    <div class="comm-head"><div>Réf.</div><div>Chantier</div><div>Artisan</div><div>Montant HT</div><div>Commission 8%</div></div>
    ${signed.map(p => `
    <div class="comm-row">
      <div class="dref">${escHtml(p.id)}</div>
      <div><div class="dname">${escHtml(p.type)}</div><div class="dmeta">${escHtml(p.client.nom)} · ${escHtml(p.ville)}</div></div>
      ...
      <div>
        ...
        <button class="abtn" style="margin-top:4px;width:auto;padding:2px 8px;font-size:.65rem;" onclick="sendRelanceSignature('${escAttr(p.id)}')">✉️ Relance</button>
      </div>
    </div>`).join('')}`;
}
...
function sendRelanceSignature(id) {
  const p = projects.find(x => x.id === id);
  if (!p || typeof EmailTemplates === 'undefined') return;
...
function exportCSV() {
  ...
  const header = ['Référence','Type travaux','Client','Téléphone','Email','Ville','CP','Surface',
                  'Budget estimé','Montant signé','Statut','Artisan assigné','Date','Commission 8%','Nb photos','Notes'].join(';');
  ...
  a.href = url; a.download = 'renolink-projets-' + todayStr() + '.csv';
...
// EXPORT PDF
// ══════════════════════════════════════════
function exportPDFFromModal() { if (AppState.activeProject) exportPDF(AppState.activeProject.id); }
...
function exportPDF(id) {
  ...
  const html = `<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><title>Dossier ${escHtml(p.id)} — RenoLink</title>
<style>
  ...
  .row:last-child{border-bottom:none;}.label{color:#7A736A;}.value{font-weight:600;}.gold{color:#B8820A;}
  ...
  footer{margin-top:24px;padding-top:12px;border-top:1px solid #E4DDD0;font-size:11px;color:#7A736A;text-align:center;}
</style></head><body>
<div class="header">
  <div><div class="logo">Reno<span>Link</span></div><div style="font-size:11px;color:#7A736A;">Lorraine</div></div>
  <div style="text-align:right;"><h1 style="font-size:18px;margin:0 0 3px;">${escHtml(p.type)}</h1>
    <div style="color:#B8820A;font-weight:700;">${escHtml(p.id)}</div>
    ...
    <div style="margin-top:6px;background:${p.statut==='signe'?'#D1FAE5':'#EFF6FF'};color:${p.statut==='signe'?'#065F46':'#1D4ED8'};font-size:11px;font-weight:700;padding:2px 10px;border-radius:100px;display:inline-block;">
      ${(statutCfg[p.statut]||{text:p.statut}).text}</div>
  </div>
</div>
<div class="section"><h2>👤 Coordonnées client — CONFIDENTIEL</h2>
  <div class="row"><span class="label">Nom</span><span class="value">${escHtml(p.client.nom)}</span></div>
  <div class="row"><span class="label">Téléphone</span><span class="value">${escHtml(formatTel(p.client.tel))}</span></div>
  <div class="row"><span class="label">Email</span><span class="value">${escHtml(p.client.email)}</span></div>
  <div class="row"><span class="label">Préférence</span><span class="value">${escHtml(p.client.pref)}</span></div>
</div>
<div class="section"><h2>🏗️ Projet</h2>
  <div class="row"><span class="label">Type</span><span class="value">${escHtml(p.type)}</span></div>
  <div class="row"><span class="label">Ville</span><span class="value">${escHtml(p.ville)} (${escHtml(p.cp)})</span></div>
  ...
  <div class="row"><span class="label">Délai</span><span class="value">${escHtml(p.delai)}</span></div>
  <div class="row"><span class="label">Date</span><span class="value">${escHtml(p.date)}</span></div>
  <div style="margin-top:10px;font-size:12px;line-height:1.65;">${escHtml(p.desc)}</div>
  ${photosHtml ? `<div class="photos-grid" style="margin-top:10px;">${photosHtml}</div>` : ''}
</div>
<div class="comm-box">
  <h3 style="color:#92400E;font-size:13px;margin-bottom:6px;">💶 COMMISSION RENOLINK — 8%</h3>
  <div style="font-size:26px;font-weight:700;color:#B8820A;">${comm.toLocaleString('fr-FR')} €</div>
</div>
${p.artisanAssigne ? `<div class="section"><h2>🔨 Artisan</h2><div class="row"><span class="label">Artisan assigné</span><span class="value">${escHtml(p.artisanAssigne)}</span></div></div>` : ''}
<div class="section"><h2>📅 Historique</h2>
  ${(p.historique||[]).map(h => `<div class="histo-item"><span style="color:#7A736A;min-width:140px;">${escHtml(h.d)}</span><span>${escHtml(h.a)}</span></div>`).join('')}
</div>
${p.notes ? `<div class="section"><h2>📝 Notes internes</h2><div style="line-height:1.65;">${escHtml(p.notes)}</div></div>` : ''}
<footer><strong>RenoLink</strong> — Artisans BTP vérifiés — Lorraine<br>contact@renolink.fr · renolink.fr</footer>
</body></html>`;

  const win = window.open('', '_blank');
  if (!win) { toast('⚠️ Autorisez les popups', 'error'); return; }
...
function sendWhatsAppModal() { if (AppState.activeProject) sendWA(AppState.activeProject.id); }
...
function sendEmailViaMailto(to, sujet, corps) {
  window.location.href = `mailto:${encodeURIComponent(to)}?subject=${encodeURIComponent(sujet)}&body=${encodeURIComponent(corps)}`;
...
function sendEmailModal() {
  const p = AppState.activeProject;
  if (!p) return;
  const tpl = (typeof EmailTemplates !== 'undefined' && EmailTemplates[p.statut])
    ? EmailTemplates[p.statut](p)
    : { sujet: `Votre projet RenoLink ${p.id}`, corps: `Bonjour ${p.client.nom.split(' ')[0]},\n\nMise à jour concernant votre projet.\n\nL'équipe RenoLink` };
...
function ouvrirFactureModal() {
  const p = AppState.activeProject;
  if (!p) return;
  if (p.statut !== 'signe') { toast('⚠️ Facturation uniquement pour chantiers signés', 'error'); return; }
  closeModal('modal-projet');
  setTimeout(() => {
...
function exportAuditLog() {
  toast('📋 Export audit — fonctionnalité en prod via Firebase', 'info');
}

// ══════════════════════════════════════════
// ARTISAN — FICHE COMPLÈTE
// ══════════════════════════════════════════
function openArtisan(id) {
  ...
  const get = (elId) => document.getElementById(elId) || { value:'', checked:false, innerHTML:'', textContent:'' };
  ...
  const badges = {
    actif:    '<span class="statut s-signe">✅ Actif</span>',
    attente:  '<span class="statut s-cours">⏳ En attente</span>',
    suspendu: '<span class="statut s-rejete">⏸️ Suspendu</span>'
  };
  get('ma-badge').innerHTML = badges[a.statut] || '';
  renderArtisanProjets(a);
  const totalComm = projects.filter(p => p.artisanAssigne === a.rs && p.statut === 'signe')
    .reduce((s,p) => s + Math.round((p.montantSigne||p.budget)*0.08), 0);
...
function renderArtisanProjets(a) {
  ...
  if (!lies.length) { el.innerHTML = '<p style="font-size:.76rem;color:var(--muted);">Aucun projet assigné.</p>'; return; }
  el.innerHTML = lies.map(p => {
    const st = statutCfg[p.statut] || { text:p.statut, cls:'' };
    return `<div style="display:flex;align-items:center;gap:12px;padding:9px 0;border-bottom:1px solid #F5F2ED;cursor:pointer;"
      onclick="closeModal('modal-artisan');setTimeout(()=>openProject('${escAttr(p.id)}'),200)">
      ...
      <span class="statut ${st.cls}">${st.text}</span>
      ${p.statut==='signe'?`<span style="font-size:.72rem;font-weight:700;color:var(--gold);">+${Math.round((p.montantSigne||p.budget)*0.08).toLocaleString('fr-FR')} €</span>`:''}
    </div>`;
  }).join('');
}
...
function saveArtisan() {
  ...
  const a  = artisans.find(x => x.id === id);
  if (!a) return;
  const rs     = sanitize((document.getElementById('ma-rs')||{value:''}).value.trim());
  const tel    = sanitize((document.getElementById('ma-tel')||{value:''}).value.trim().replace(/\s/g,''));
  const email  = sanitize((document.getElementById('ma-email')||{value:''}).value.trim());
  const metier = sanitize((document.getElementById('ma-metier')||{value:''}).value.trim());
  const ville  = sanitize((document.getElementById('ma-ville')||{value:''}).value.trim());
  if (!rs||!tel||!metier||!ville) { toast('⚠️ Champs obligatoires manquants','error'); return; }
  if (email && !isValidEmail(email)) { toast('⚠️ Email invalide','error'); return; }
  a.rs=rs; a.siret=sanitize((document.getElementById('ma-siret')||{value:''}).value.trim());
  a.metier=metier; a.ville=ville; a.tel=tel; a.email=email;
  a.zone=sanitize((document.getElementById('ma-zone')||{value:''}).value.trim());
  a.notes=sanitize((document.getElementById('ma-notes')||{value:''}).value);
  a.kbis=(document.getElementById('ma-kbis')||{checked:false}).checked;
  a.decennale=(document.getElementById('ma-decennale')||{checked:false}).checked;
  a.rcpro=(document.getElementById('ma-rcpro')||{checked:false}).checked;
  a.rge=(document.getElementById('ma-rge')||{checked:false}).checked;
  a.qualibat=(document.getElementById('ma-qualibat')||{checked:false}).checked;
  a.qualipac=(document.getElementById('ma-qualipac')||{checked:false}).checked;
  a.statut=(document.getElementById('ma-statut')||{value:'attente'}).value;
  AppState.activeArtisan = a;
  ...
  if (badge) badge.innerHTML = badges[a.statut] || '';
  const title = document.getElementById('ma-title');
  if (title) title.textContent = a.rs;
  renderArtisans(); renderKPIs();
...
function reactiverArtisan() {
  ...
  if (!a) return;
  a.statut = 'actif';
  const sel = document.getElementById('ma-statut');
  if (sel) sel.value = 'actif';
  const badge = document.getElementById('ma-badge');
  if (badge) badge.innerHTML = '<span class="statut s-signe">✅ Actif</span>';
  renderArtisans(); renderKPIs();
